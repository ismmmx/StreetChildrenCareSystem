...
namespace StreetChildrenCareSystem.Forms
{
    public partial class frmVaccination : Form
    {
        ...
        {
            // e.RowIndex -1 means header was clicked, so ignore it
            ...
            try
            {
                ...
                // Store selected VacID safely
                if (row.Cells["VacID"].Value != null && row.Cells["VacID"].Value != DBNull.Value)
                    selectedVacID = Convert.ToInt32(row.Cells["VacID"].Value);

                // Fill form fields from selected row
        ...
        {
            // Read directly from the selected row - more reliable than stored variable
            ...
            // Get VacID directly from the currently highlighted row
            selectedVacID = Convert.ToInt32(dgvVaccination.CurrentRow.Cells["VacID"].Value);
        ...
        private void cmbVacStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
